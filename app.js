...
function startQuiz(){
  ...
  score=0; current=0; userAnswers=[]; order=[...QUESTIONS.keys()]; if(el('shuffleToggle').checked) shuffle(order);
...
function renderCertificate(pct){ const title=document.getElementById('app-title')?.textContent||'クイズ'; const total=QUESTIONS.length; el('certName').textContent=studentName||'受講者'; el('certCourse').textContent=title; el('certScore').textContent=`${score} / ${total}（${pct}%）`; el('certDate').textContent=getNowString(); el('certPassLine').textContent=`${PASS_PERCENT}%`; const id=generateCertId(); el('certId').textContent=id; try{ localStorage.setItem('quiz-cert-id', id);}catch{} }
...
const certBtnEl=document.getElementById('certBtn'); if(certBtnEl){ certBtnEl.addEventListener('click', ()=>{ document.body.classList.add('print-cert'); setTimeout(()=>{ window.print(); setTimeout(()=>document.body.classList.remove('print-cert'), 500); }, 50); }); }
