...
function startQuiz(){
  ...
  const orgInput = document.getElementById('studentOrg');
  const val = nameInput ? nameInput.value.trim() : '';
  if(!val){ alert('受講者氏名を入力してください'); return; }
  studentName = val; try{ localStorage.setItem('quiz-name', studentName);}catch{}
  studentOrg = orgInput ? orgInput.value.trim() : ''; try{ localStorage.setItem('quiz-org', studentOrg);}catch{}
...
try{ const saved=localStorage.getItem('quiz-name'); if(saved){ const ni=document.getElementById('studentName'); if(ni) ni.value=saved; } const savedOrg=localStorage.getItem('quiz-org'); if(savedOrg){ const oi=document.getElementById('studentOrg'); if(oi) oi.value=savedOrg; } }catch{}
